...
for(var i=0; i < comment.length; i++){ 
  comment[i].onclick = function(event){
    ...
    var userid = (event.target && event.target.getAttribute('data-c-user_id'));
      if(userid == "")
      { window.location.replace("http://192.168.99.100:8080/users/login");
      }
    ...
    xhttp.open('POST', 'http://192.168.99.100:8080/Posts/comment');
...
function like(event)
{
  ...
  var xhttp = new XMLHttpRequest();
  xhttp.open('POST', 'http://192.168.99.100:8080/Posts/Like');
  xhttp.withCredentials = true;
